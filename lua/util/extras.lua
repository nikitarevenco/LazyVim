...
---@class lazylsp.util.extras
local M = {}
M.buf = 0

---@type LazyExtraSource[]
M.sources = {
  { name = "LazyLsp", desc = "LazyLsp extras", module = "lazylsp.plugins.extras" },
  { name = "User", desc = "User extras", module = "plugins.extras" },
}

M.ns = vim.api.nvim_create_namespace("lazylsp.extras")
---@type string[]
M.state = nil
...
function M.wants(opts)
  ...
  if opts.root then
    ...
    return #LazyLsp.root.detectors.pattern(M.buf, opts.root) > 0
  end
  return false
end
...
function M.get()
  ...
  for _, source in ipairs(M.sources) do
    local root = LazyLsp.find_root(source.module)
    if root then
      LazyLsp.walk(root, function(path, name, type)
...
function X.new()
  ...
  self.float = Float.new({ title = "LazyLsp Extras" })
...
function X:toggle()
  ...
  for _, extra in ipairs(self.extras) do
    if extra.row == pos[1] then
      ...
      LazyLsp.config.json.data.extras = vim.tbl_filter(function(name)
        return name ~= extra.module
      end, LazyLsp.config.json.data.extras)
      M.state = vim.tbl_filter(function(name)
        return name ~= extra.module
      end, M.state)
      if extra.enabled then
        table.insert(LazyLsp.config.json.data.extras, extra.module)
        M.state[#M.state + 1] = extra.module
      end
      table.sort(LazyLsp.config.json.data.extras)
      LazyLsp.json.save()
      LazyLsp.info(
        "`"
          .. extra.name
          ...
          .. "\nPlease restart LazyLsp to apply the changes.",
        { title = "LazyExtras" }
      )
      self:update()
      return
...
function X:render()
  self.text:nl():nl():append("LazyLsp Extras", "LazyH1"):nl():nl()
  self.text
    :append("This is a list of all enabled/disabled LazyLsp extras.", "LazyComment")
    :nl()
...
function X:extra(extra)
  ...
  if extra.source.name ~= "LazyLsp" then
  ...
  for _, import in ipairs(extra.imports) do
    import = import:gsub("^lazylsp.plugins.extras.", "")
